...
void handleStatusBattery() {
  bat_voltage = analogRead(34) * 3.3 * 5.7 / 4096;
  char status_bat[50];
  sprintf(status_bat, "%.2fV", bat_voltage);
  server.send(200, "text/plain", status_bat);
}
...
void setup() {
  ...
  server.on("/battery", HTTP_GET, handleStatusBattery);
